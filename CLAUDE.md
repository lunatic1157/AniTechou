...
- **5 状态**: 想看/在看/看过/搁置/抛弃
- **分页加载**: 每页 50 部
- **Markdown 笔记**: Markdig AST walker 预览引擎
- **AI 本地管理**: BuildCollectionContext 汇总收藏数据
- **Bangumi 同步增强**: 5 状态映射、类型映射、用户标签导入、infobox 解析、已有作品元数据补全

## v0.9.5 新特性
- **MD-native 编辑器**: 底层存储=Markdown，编辑=源码+实时预览，删除 XAML↔MD 双模式切换
- **工具栏 MD 语法插入**: B→`**`, I→`*`, U→`<u>`, H1→`# `, 🔗→`[text](url)`, 🖼→`![](path)` 等
- **HTML 标签渲染**: `<u>`→下划线, `<span style="color:...">`→文字颜色/高亮
- **图片渲染**: `![](path)` 本地图片直接显示在预览中（InlineUIContainer）
- **链接渲染**: 蓝色下划线 + 点击浏览器打开
